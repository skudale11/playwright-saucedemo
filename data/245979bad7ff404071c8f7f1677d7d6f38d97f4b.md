...
- Name: cart.spec.ts >> user can complete full checkout
- Location: tests/cart.spec.ts:19:5
...
Call log:
  - waiting for locator('.btn_add_to_cart').first()
...
```ts
  ...
  5  | 
  6  |   async sortBy(option: string) {
  7  |     await this.page.locator('[data-test="product-sort-container"]').selectOption(option);
  8  |   }
  9  | 
  10 |   async addFirstProductToCart() {
> 11 |     await this.page.locator('.btn_add_to_cart').first().click();
     |                                                         ^ Error: locator.click: Test timeout of 30000ms exceeded.
  12 |   }
  13 | 
  14 |   async getCartCount() {
  15 |     return this.page.locator('.shopping_cart_badge');
  16 |   }
  17 | }
```